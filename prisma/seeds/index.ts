...
// Every seeded role except super_admin (which bypasses via the isSuperAdmin flag).
const NON_SUPER_ROLES = ROLE_KEYS.filter((k) => k !== "super_admin");
...
export async function seedSystem(db: PrismaClient): Promise<void> {
  ...
  // NOTE: create-only/idempotent — this seed never prunes. Removing a catalog
  // entry leaves existing DB rows; intentional removals need a separate migration.
  ...
  for (const m of MODULES) {
    for (const roleKey of NON_SUPER_ROLES) {
      await db.moduleRoleAccess.upsert({
        ...
        update: {},
        create: { moduleKey: m.key, roleKey, canAccess: true },
      });
    }
  }
  ...
  for (const roleKey of NON_SUPER_ROLES) {
    const grants = deriveRolePermissions(roleKey.toUpperCase() as keyof typeof roleCapabilities);
    for (const permissionKey of grants) {
      await db.rolePermission.upsert({
        where: { roleKey_permissionKey: { roleKey, permissionKey } },
        update: {},
        create: { roleKey, permissionKey, granted: true },
      });
    }
  }
