/**
 * 301 redirects from the old DoctorLogic slugs to the new IA (B3).
 * Source of truth: REDIRECTS.md. Old slugs must keep working indefinitely (SEO).
 *
 * Shared by next.config.ts (to register the redirects) and tests/redirects.mjs
 * (to assert each one still 3xx-es), so the two never drift.
 */
